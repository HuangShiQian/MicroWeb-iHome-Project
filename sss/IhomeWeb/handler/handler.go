...
import (
	...
	PUTUSEREINFO    "sss/PutUserInfo/proto/example"
...
func GetArea(w http.ResponseWriter, r *http.Request,ps httprouter.Params) {
	...
	// call the backend service
	//3.通过protobuf 生成文件 创建 连接服务端 的客户端句柄
...
func GetImageCd(w http.ResponseWriter, r *http.Request,ps httprouter.Params) {
	...
	fmt.Println("获取图片验证码  GetImageCd  /api/v1.0/imagecode/:uuid")
...
*/
	...
	// call the backend service
	//通过protobuf 生成文件 创建 连接服务端 的客户端句柄  -------注意此处就要导包了
...
func GetUserInfo(w http.ResponseWriter, r *http.Request,ps httprouter.Params) {
	...
	data["avatar_url"] = utils.AddDomain2Url(rsp.AvatarUrl)//  怎么理解?
...
func PutUserInfo(w http.ResponseWriter, r *http.Request,ps httprouter.Params) {
	...
	// call the backend service
	//通过protobuf 生成文件 创建 连接服务端 的客户端句柄
	exampleClient := PUTUSEREINFO.NewExampleService("go.micro.srv.PutUserInfo", cli.Client())
	//通过句柄调用服务端函数
	rsp, err := exampleClient.PutUserInfo(context.TODO(), &PUTUSEREINFO.Request{
		Name: request["name"].(string),
		Sessionid:cookie.Value,
	})
	//判断是否成功
